...
local character, humanoid, rootPart, equipped, down, stance
-- REMOVED: 'flightForce' variable
...
local function Dash()
	if character then
		-- REMOVED: Check for 'flying'
		if canRoll and (not grinding) and (not humanoid.Down.Value) then
			canRoll	= false
...
RunService:BindToRenderStep("Control", 3, function(deltaTime)
	if character and humanoid.Health > 0 then
		...
		-- REMOVED: Entire 'if flying then ...' block
		...
		if grinding then
			...
			-- REMOVED: 'if not rootPart:FindFirstChild("SpaceshipWeld") then'
			-- This code will now run by default
...
-- REMOVED: Entire 'REMOTES.Deploy.OnClientEvent' function

INPUT.ActionBegan:connect(function(action, processed)
	if not processed then
		if action == "Jump" then
			if character and humanoid.Health > 0 then
				-- REMOVED: 'SpaceshipWeld' check
				-- The 'else' block is now the default
				if grinding then
					grinding		= false
					railCooldown	= RAIL_COOLDOWN
					character.Animate.Grind:Fire(false)
