...
local HORN_SIZE		= Vector3.new(0.7, 0.7, 0.7)
local HORN_CENTER	= Vector3.new(0, 0.7, 0)
local HORN_SCALE	= 0.3

local WING_SIZE		= Vector3.new(0.4, 0.4, 0.4)
...
return function(character)
	...
	local level, wings, horns	= effects:FindFirstChild("AngelLevel"), effects:FindFirstChild("AngelWings"), effects:FindFirstChild("AngelHalo")
	
	if not level then
		level	= Instance.new("IntValue")
			level.Name		= "AngelLevel"
			level.Value		= 0
			level.Parent	= effects
	end
	if not wings then
		wings	= script.AngelWings:Clone()
