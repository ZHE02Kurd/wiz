		...
		if slot.Item then
			...
			local function UpdateCFrame()
				...
				if item.Name == "Armor" then
					-- NEW ORIENTATION FOR ARMOR PLATE ONLY
					-- (Set your desired rotation here)
					local y_rotation = math.pi / 2 -- 90 degrees
					local x_rotation = math.pi / 6 -- 30 degrees
					preview:SetPrimaryPartCFrame(CFrame.new(0, 0, 8 + dist * 4) * CFrame.Angles(0, y_rotation, 0) * CFrame.Angles(x_rotation, 0, 140) * offset)
				else
					-- DEFAULT ORIENTATION FOR ALL OTHER ITEMS
...
local function SetSlot(newSlot, autoUse)
	currentSlot	= newSlot
	...
	if enabled and (not healing) then
		if down then
			if down.Value then
				...
				if autoUse and slots[currentSlot] and slots[currentSlot].Item then
					task.spawn(function()
						local item = slots[currentSlot].Item
						local config = require(item:WaitForChild("Config"))

						if config and config.Type == "Booster" and config.Boost == "Health" then
							REMOTES.InstantBooster:FireServer(item)
						else
...
UserInputService.InputChanged:connect(function(inputObject, processed)
	if not processed then
		if inputObject.UserInputType == Enum.UserInputType.MouseWheel then
			-- Calculate new slot based on scroll direction
			local newSlot = currentSlot - inputObject.Position.Z

			-- Wrap around the backpack size
			...
			SetSlot(newSlot, false) -- Always pass FALSE for no auto-use
		end
	end
end)
