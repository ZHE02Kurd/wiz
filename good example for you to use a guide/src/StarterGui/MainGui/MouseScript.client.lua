...
local function Hitmarker(position, damage, armor, headshot)
	...
	tweenB:Play()
	-- Wait for the fade-out tween to finish, then destroy
	tweenB.Completed:Connect(function()
		if hitmarker then hitmarker:Destroy() end
	end)
	...
	tweenC:Play()
	tweenD:Play()
	-- Wait for the movement tween to finish, then destroy
	tweenD.Completed:Connect(function()
		if damageLabel then damageLabel:Destroy() end
	end)
