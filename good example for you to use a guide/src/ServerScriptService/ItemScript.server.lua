...
local function Unequip(item) -- move an item from the hand to the back
	spawn(function() -- As you noted, this runs the code in a new thread
		if item:IsDescendantOf(Workspace) then
			...
			-- remove equipped stuff (e.g., Motor6D from the hand)
			local gripMotor = handle:FindFirstChild("GripMotor")
			if gripMotor then
				gripMotor:Destroy()
			end

			local position = GetPosition(item) -- Assuming GetPosition() is a function you've defined
			local attachment

			-- Find the base attachment point on the torso
			attachment = character[position .. "Torso"][item.CharacterSlot.Value]

			-- Check for an override attachment (e.g., on a backpack)
			...
			local weld = Instance.new("Weld") -- Fixed: Added missing ')'
			weld.Name = "UnequippedWeld"
			weld.Part0 = attachment.Parent -- Fixed: Corrected typo 'Pare' -> 'Parent'
			weld.Part1 = handle -- Fixed: Used 'handle' instead of undefined 'hand'

			-- Apply custom back rotation if specified in config
			local config = require(item.Config) -- Fixed: Added missing ')'
			local rotationCFrame = CFrame.new() -- Fixed: Added '()' to create identity CFrame

			if config.BackRotation then -- Fixed: 'the' -> 'then'
				rotationCFrame = CFrame.Angles( -- Fixed: 'Angle' -> 'Angles' and added '()'
					math.rad(config.BackRotation.X or 0), -- Fixed: Added 'or 0' and comma
					math.rad(config.BackRotation.Y or 0), -- Fixed: Added 'or 0' and comma
					math.rad(config.BackRotation.Z or 0) -- Fixed: Added 'or 0'
				)
			end -- Fixed: 'en' -> 'end'

			-- Set the weld offsets
			weld.C0 = attachment.CFrame * rotationCFrame -- Fixed: 'rotationCFr' -> 'rotationCFrame'

			-- Assuming 'Center' is an Attachment or CFrameValue inside the handle
			if handle:FindFirstChild("Center") then
				weld.C1 = handle.Center.CFrame -- Fixed: 'CFr' -> 'CFrame'
			else
				-- Fallback if 'Center' doesn't exist
				weld.C1 = CFrame.new() 
			end

			weld.Parent = handle -- Fixed: Used 'handle' instead of undefined 'hand'
		end
	end)
end
...
local function Equip(item) -- move an item from the back to the hand
	spawn(function()
		...
		if not handle:FindFirstChild("Grip") then return end
